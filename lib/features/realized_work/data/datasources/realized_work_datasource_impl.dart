...
class RealizedWorkDatasourceImpl extends RealizedWorkDatasource {
  ...
  Future<void> deleteWork(String id) {
    try {
      return dio.delete('/servicio/ejemplo/$id');
    } catch (e) {
      throw Exception(e);
    }
  }
  ...
  Future<Works> getRealizedWorkById(String id) async {
    try {
      final response = await dio.get('/servicio/ejemplo/$id');
      Works work = Works(
        id: '0',
        name: 'No encontrado',
        description: 'No encontrado',
  ...
  Future<List<Works>> getRealizedWorks() async {
    try {
      final response = await dio.get('/servicio/ejemplo');
